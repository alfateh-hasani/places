...
use App\Services\Pricing\PricingService;    // ← إضافة
use Config;
...
class ApartmentController extends Controller
{
    ...
    protected PricingService $pricing;         // ← إضافة

    public function __construct(Apartment $apartment, PricingService $pricing)
    {
        $this->apartment = $apartment;
        $this->pricing   = $pricing;           // ← إضافة
    }
    ...
    public function search(Request $request)
    {
        ...
        foreach ($filters as $key => $val) {
            ...
            if ($key === 'check_in' || $key === 'check_out') {
                if (!isset($filters['check_in'], $filters['check_out'])) continue;
                ...
                $query->whereDoesntHave('bookings', fn($q) =>
                    $q->where('check_in', '<', $co)->where('check_out', '>', $ci)
                );
                break;
            }
